class MusicVisualizer {
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('.color-option').forEach(option => {
            option.addEventListener('click', (e) => {
                ...
                console.log('Color theme changed to:', this.colorTheme);
            });
        });
    ...
    drawVisualizer(transparent = false) {
        ...
        switch(this.visualMode) {
            case 'bar': this.drawBars(); break;
            case 'wave': this.drawWaveform(); break;
            case 'circle': this.drawCircularVisualizer(); break;
            case 'particle': this.drawParticles(); break;
            case 'spectrum': this.drawSpectrum(); break;
            case 'spiral': this.drawSpiral(); break;
            case 'mirror': this.drawMirror(); break;
    ...
    drawFlower() {
        ...
        for (let petal = 0; petal < 8; petal++) {
            ...
            for (let i = 0; i < this.bufferLength; i++) {
                ...
                const intensity = (this.dataArray[i] / 255) * this.sensitivity;
                const radius = intensity * 200 * Math.sin(angle * 3 + petal * Math.PI / 4);
                
                const x = centerX + Math.cos(angle + petal * Math.PI / 4) * Math.abs(radius);
                const y = centerY + Math.sin(angle + petal * Math.PI / 4) * Math.abs(radius);
    ...
    drawTunnel() {
        ...
        for (let layer = 0; layer < 10; layer++) {
            const avgFreq = this.dataArray.slice(layer * 10, (layer + 1) * 10).reduce((a, b) => a + b, 0) / 10;
            const intensity = (avgFreq / 255) * this.sensitivity;
            ...
            this.ctx.strokeStyle = `rgba(${color.r}, ${color.g}, ${color.b}, ${0.8 - layer * 0.08})`;
            this.ctx.lineWidth = this.lineWidth * 2;
            this.ctx.stroke();
        }
    }
    ...
    drawGrid() {
        ...
        for (let x = 0; x < cols; x++) {
            for (let y = 0; y < rows; y++) {
                const index = (x + y) % this.bufferLength;
                ...
                if (intensity > 0.3) {
                    ...
                    this.ctx.fillStyle = `rgba(${color.r}, ${color.g}, ${color.b}, ${intensity})`;
                    this.ctx.fillRect(x * gridSize, y * gridSize, gridSize * intensity, gridSize * intensity);
                }
            }
        }
    }
    ...
    drawKaleidoscope() {
        ...
        for (let segment = 0; segment < 8; segment++) {
            ...
            this.ctx.rotate((segment * Math.PI * 2) / 8);
            
            for (let i = 0; i < this.bufferLength; i++) {
                const intensity = (this.dataArray[i] / 255) * this.sensitivity;
                const distance = intensity * 300;
                
                const colorIndex = i % colors.length;
                const color = this.hexToRgb(colors[colorIndex]);
                
                this.ctx.beginPath();
                this.ctx.arc(distance, 0, intensity * 10, 0, 2 * Math.PI);
                this.ctx.fillStyle = `rgba(${color.r}, ${color.g}, ${color.b}, ${intensity})`;
                this.ctx.fill();
            }
    ...
    drawLightning() {
        ...
        for (let bolt = 0; bolt < 5; bolt++) {
            const startX = (bolt / 5) * width;
            const avgFreq = this.dataArray.slice(bolt * 20, (bolt + 1) * 20).reduce((a, b) => a + b, 0) / 20;
            const intensity = (avgFreq / 255) * this.sensitivity;
    ...
    drawRipple() {
        ...
        for (let i = 0; i < this.bufferLength / 10; i++) {
            const intensity = (this.dataArray[i * 10] / 255) * this.sensitivity;
            const radius = intensity * 400 + this.time * 50 + i * 30;
            
            const colorIndex = i % colors.length;
            const color = this.hexToRgb(colors[colorIndex]);
            
            this.ctx.beginPath();
            this.ctx.arc(centerX, centerY, radius % 600, 0, 2 * Math.PI);
            this.ctx.strokeStyle = `rgba(${color.r}, ${color.g}, ${color.b}, ${intensity * 0.5})`;
            this.ctx.lineWidth = this.lineWidth;
            this.ctx.stroke();
        }
    }
    ...
    drawCube() {
        ...
        edges.forEach((edge, index) => {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(projected[edge[0]][0], projected[edge[0]][1]);
            this.ctx.lineTo(projected[edge[1]][0], projected[edge[1]][1]);
            this.ctx.strokeStyle = `rgba(${color.r}, ${color.g}, ${color.b}, ${intensity})`;
            this.ctx.lineWidth = this.lineWidth * 2;
            this.ctx.stroke();
        });
